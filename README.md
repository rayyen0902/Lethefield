...
开发执行依据见《Lethefield-开发文档》v1.2。**当前进度：M0–M12 已完成（CI 全绿），
下一模块 M13（多租户工程红线落地）。**
...
```bash
bash scripts/ci.sh   # = lint + 单测 + 起全栈 + 集成测试（q1–q4 基线 + M1–M12 验收）
```
...
# M10 迁移演练（按需，不占常驻内存）：第二 Cell profile 起栈后跑 drill
docker compose --profile cell2 up -d && bash scripts/wait_for_stack.sh
uv run pytest tests/integration/test_m10_migration_drill.py -v
```
...
libs/clients/      存储/Pulsar 客户端封装 + ControlPlaneStore 抽象（M0 冻结接口）+ 契约 5
...
ops/metrics_exporter/ 离线聚合指标 worker（M12：日志管线/元数据 → Prometheus 指标）
services/          服务进程边界：api（M5 接口层）/ fs（M6 sweep）/ rms（M2–M4/M7）/
                   scheduler（M9/M10 调度器）/ training（M11/M12 训练数据管线 + ③ 过滤器）
tests/integration/ spike q1–q4 CI 集成基线 + M1–M12 各模块验收测试
docker-compose.yml 单节点全栈：JanusGraph + Cassandra×2 + ES×2 + Pulsar + Redis + PostgreSQL
                   + Prometheus(9090) + Grafana(3000)（M12，dashboard 已 provision）
...
M2–M12 各模块巡检/运维命令（schema 巡检、FS liveness、纠错、重建、调度器 CLI、
ingest DMS、训练管线 worker/scrub/ex-feed、recall-filter、metrics_exporter 等）

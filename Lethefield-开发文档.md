...
**阶段状态（2026-08-23 补记）**：阶段 0/1 全部模块（M0–M17）已交付，阶段 1 技术落地验证**已准出闭合**（2026-08-15 所有者签署，基线标签 `stage-1-exit`，见《准出评审报告-v1_0.md》官宣栏与《工作日志.md》收官节）。当前处于阶段 2（种子期）规划期（《规划-种子期计划-v0_1.md》）。本文档进入冻结维护态：既有模块内容不再扩写，种子期参数标定结果回填 §20，设计层变更走升级确认 + 修订记录；已知边界清单（§20 待标定参数、API 多 Cell 连接路由、告警空白 6 条、观察项 7 条）已全部汇入种子期计划 §2。
...
- **阶段 0/1（本次开发范围）**：自有小规模集群，技术落地验证。包含：§7 技术栈、§15 存储物理隔离目标架构在自有集群落地、§17 Cell 架构、§18 EX/Pulsar 归属、§9 对外 API 接口（v1.8 收窄为 HTTP+JSON）、§13 记忆维护机制、§19 可观测性最小集、§12.4 训练管线最小集。
...
## 6. M5 — 对外 API 层（HTTP+JSON）

### 目标
落地 §9 定稿的四类操作接口，鉴权字段来源必须是凭证本身。

> **v1.8 收窄（设计文档 v1.8 同步）**：本模块正式对外契约 = HTTP + JSON API；MCP 封装降级为远期可选（既有 stdio 薄壳 `mcp_server.py` 保留为本地/开发便利形态，非正式契约，不再投入）；SDK 属前端工程薄封装、不设独立后端模块。
>
> ~~原遗留批注（"Codex/Claude Code 没有实时写入事件……不能依赖 LLM 主动上传；MCP 说明书后期独立做"）~~——**已勾除（2026-08-23）**：该问题被《前端设计-客户端与宿主接入》§2.1 正式化（认知必然性原则 + 决策 D1/D3/D4），被动上传与宿主接入（dsh Cordis 插件 / Codex app-server 适配器）归前端工程，见该文档 §3–§5。
